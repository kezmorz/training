      ...
      persons = (
        <div>
          {this.state.persons.map((person, index) => {     // map will execute whatever function we define on every element in the array (person can be named whatever we want)
            return (
              // key always has to be on the outer element when using the map method
              <ErrorBoundary key={person.id}     // key is a default property that React expects on a list item (helps react update the Virtual DOM by being able to compare elements from the past to the future)
              >
                <Person 
